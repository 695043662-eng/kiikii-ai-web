...
export default function FabricTextLayer({
  ...
  useEffect(() => {
    ...
    // #437 修复：事件监听器函数定义移到外部，确保 cleanup 可以访问
    ...
    if (canvasContainer) {
      ...
      Object.assign((canvasContainer as HTMLDivElement).style, {
        ...
        // 注意：不设置 zIndex，改用全局 CSS 选择器设置，避免 React 19 insertBefore 错误
      });
    ...
    return () => {
      // #437 修复：清理事件监听器，防止内存泄漏
      if (canvasContainer) {
        canvasContainer.removeEventListener('contextmenu', handleContainerContextMenu);
      }
      if (upperCanvas) {
  ...
  useEffect(() => {
    ...
    textElements.forEach((el) => {
      ...
      if (!textObj) {
        ...
        // 检查尺寸是否由外部调整（如画布选中框拉伸）
        const oldWidth = textObj.width || 0;
        const oldHeight = textObj.height || 0;
        const newWidth = el.width || 0;
        const newHeight = el.height || 0;
        const sizeChangedByExternal = (Math.abs(newWidth - oldWidth) > 2 || Math.abs(newHeight - oldHeight) > 2) && fontSizeChanged;
        ...
        // 如果是外部拉伸导致的字号变化（选中框拉伸），不重新计算尺寸，直接使用外部指定的尺寸
        // 只有在 Fabric.js 内部操作（如直接拖拽 Fabric 控制点）导致的字号变化时才重新计算
        if (fontSizeChanged && !sizeChangedByExternal && !isUpdatingFromFabric.current) {
          const text = textObj.text ?? '';
          const charWidth = newFontSize * 0.6;
          const lines = text.split('\n');
          const maxLineLength = Math.max(...lines.map((l: string) => l.length), text.length || 1);
          const recalculatedWidth = Math.max(maxLineLength * charWidth + 16, 50);
          const recalculatedHeight = Math.max(lines.length * newFontSize * 1.4 + 8, newFontSize * 1.4 + 8);
