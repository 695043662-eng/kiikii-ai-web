...
/**
 * 分辨率数值排序函数（从小到大：480p → 720p → 1080p）
 */
function sortResolutionsByValue(resolutions: string[]): string[] {
  return resolutions.sort((a, b) => {
    // 提取数值部分（如 "720p" → 720, "1K" → 1, "4K" → 4）
    const getNum = (s: string): number => {
      const match = s.match(/(\d+)/);
      if (match) {
        const num = parseInt(match[1]);
        // 处理 K 单位（1K=1000, 2K=2000, 4K=4000）
...
function ShowcaseReviewInlinePanel({ adminDarkMode }: { adminDarkMode: boolean }) {
  const [pendingCards, setPendingCards] = useState<ReviewPendingCard[]>([]);
  ...
  const loadPending = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/showcase/pending?status=${reviewFilter}`, { credentials: 'include' });
      const data = await res.json();
      if (data.success) {
        setPendingCards(data.items || []);
      }
    } catch (err) {
      console.error('[ReviewInlinePanel] 加载失败:', err);
    } finally {
      setLoading(false);
    }
  }, [reviewFilter]);
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await fetch('/api/showcase/review', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ cardId: rejectId, action: 'reject', rejectReason: rejectReason || '内容不符合展示要求' }),
      });
      const data = await res.json();
      if (data.success) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">审核用户投稿的展示内容，通过后将显示在首页展示区</p>
          </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs transition-colors ${
                    reviewFilter === f
                      ...
                      : adminDarkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-background hover:bg-muted'
                  }`}
                >
      ...
      <CardContent>
        ...
        {loading && pendingCards.length === 0 ? (
          ...
          <Alert>
            ...
            <AlertDescription>
              {reviewFilter === 'pending' ? '暂无待审核内容' : reviewFilter === 'approved' ? '暂无已通过内容' : '暂无已拒绝内容'}
            </AlertDescription>
          </Alert>
        ) : (
          <div className="space-y-3">
            {pendingCards.map(card => (
              <div
                key={card.id}
                className={`p-4 rounded-xl border ${adminDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-200'} flex gap-4`}
              >
                {/* 预览图 - 更大以便审核 */}
                <div className="w-40 h-40 rounded-lg overflow-hidden bg-muted flex-shrink-0 border border-border">
                  {card.imageUrl ? (
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  {/* 标题行 */}
                  {card.title && <p className="text-sm font-semibold truncate">{card.title}</p>}
                  ...
                  {/* 提示词 */}
                  {card.prompt && (
                    <p className="text-xs text-muted-foreground line-clamp-2" title={card.prompt}>
                      提示词: {card.prompt}
                    </p>
                  )}
                  ...
                  {card.status === 'rejected' && card.rejectReason && (
                    <p className="text-xs text-destructive">拒绝原因: {card.rejectReason}</p>
                  )}
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    ...
                    <span>{card.sourceType === 'user_submission' ? '用户投稿' : card.sourceType}</span>
                  </div>
                </div>
        ...
        {rejectId && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center">
            <div className={`rounded-xl shadow-xl p-6 w-96 space-y-4 ${adminDarkMode ? 'bg-gray-900' : 'bg-card'}`}>
              <h3 className="text-sm font-semibold">拒绝原因</h3>
              <textarea
                value={rejectReason}
                onChange={e => setRejectReason(e.target.value)}
                placeholder="请输入拒绝原因（将通知用户）"
                className={`w-full h-24 px-3 py-2 rounded-lg border text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 ${adminDarkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-background border-input'}`}
              />
...
/**
 * #681 从 API 配置名称提取服务商简称
 * 支持多种格式：
 * - "图片生成 - GRS AI" → "GRS"
 * - "GRS Nano-banana" → "GRS"（开头的大写单词）
 * - "T8Star-GPT-Image-2" → "T8Star"（第一个连字符前的内容）
 * - "LingYa-HappyHorse" → "LingYa"
 * - "灵芽API" → "灵芽"
 */
function extractProviderShortName(configName: string): string {
  if (!configName) return '';
  
  // 1. 优先匹配 "空格-空格" 格式（如 "图片生成 - GRS AI"）
  const spacedDashMatch = configName.match(/\s+-\s+(.+)$/);
  if (spacedDashMatch) {
    const afterDash = spacedDashMatch[1].trim();
    // 如果包含 "AI" 后缀，去掉
    return afterDash.replace(/\s*AI$/i, '').trim() || afterDash;
  }
  
  // 2. 匹配开头的大写单词（如 "GRS Nano-banana" → "GRS"）
  const leadingWord = configName.match(/^([A-Z][A-Za-z]*)\s/);
  if (leadingWord) {
    return leadingWord[1].trim();
  }
  
  // 3. 匹配第一个连字符前的内容（如 "T8Star-GPT-Image-2" → "T8Star"）
  // 支持包含数字的名称（如 T8Star）
  const firstHyphenMatch = configName.match(/^([A-Za-z0-9]+)-/);
  if (firstHyphenMatch) {
    return firstHyphenMatch[1].trim();
  }
  
  // 4. 匹配 "XXX API" 格式（如 "灵芽API"）
  const apiMatch = configName.match(/^(.+?)\s*API/i);
  if (apiMatch) return apiMatch[1].trim();
...
// 视频模型支持的时长范围（4-15秒）
const VIDEO_DURATIONS = Array.from({ length: 12 }, (_, i) => ({
  label: `${i + 4}秒`,
  value: `${i + 4}`,
}));
...
function inferParameters(credits: number, modelType: string, config?: any, modelKey?: string): any {
  ...
  const isBanana2Series = modelKey && ['nano-banana-2', 'nano-banana-2-cl'].includes(modelKey.toLowerCase());
  ...
  if (config?.resolutions && Array.isArray(config.resolutions) && config.resolutions.length > 0) {
    if (modelType === 'video_generation') {
      // 视频模型：resolutions 是每秒积分（按分辨率）- 不再硬编码480p，直接使用模型配置
      const videoResolutions = config.resolutions;
      const defaultPricing: Record<string, number> = {};
      videoResolutions.forEach((r: any) => { defaultPricing[r.label || r.value] = r.credits || 80; });
      return {
        resolutions: videoResolutions,
        videoPricing: config.videoPricing || defaultPricing,
        aspectRatios: VIDEO_ASPECT_RATIOS,
        durations: config.durations || VIDEO_DURATIONS,
      };
    }
  ...
  if (modelType === 'video_generation') {
    // 默认视频分辨率配置（不再包含480p，因为大多数视频模型不支持）
    const defaultVideoResolutions = [
      { label: '720p', value: '720p', credits: 80 },
      { label: '1080p', value: '1080p', credits: 100 },
    ];
    return {
      resolutions: defaultVideoResolutions,
      videoPricing: { '720p': 80, '1080p': 100 },
      aspectRatios: VIDEO_ASPECT_RATIOS,
...
interface ModelCreditsConfig {
  ...
  config_id?: number; // #680 添加 config_id 用于获取服务商名称
}
...
export default function AdminDashboard() {
  ...
  // 熔断详情
  const [circuitBreakerBans, setCircuitBreakerBans] = useState<Array<{
    keyPrefix: string;
    resolution: string;
    bannedAt: number;
    banDuration: number;
  ...
  const [paymentMaintenanceLoading, setPaymentMaintenanceLoading] = useState(false);
  ...
  const getModelType = (modelKey: string | null): 'image' | 'video' | 'tool' => {
    ...
    const key = modelKey.toLowerCase();
    if (key.includes('sora') || key.includes('veo') || key.includes('video') || key.includes('sdols') || key.includes('seedance') || key.includes('kling') || key.includes('happyhorse')) return 'video';
    // LLM 模型归类为 tool（不区分分辨率）
    if (key.includes('gemini') || key.includes('gpt-5') || key.includes('deepseek') || key.includes('qwen') || key.includes('smart_split') || key.includes('split') || key.includes('upscale') || key.includes('enhance')) return 'tool';
    return 'image';
  };
  ...
  useEffect(() => {
    ...
    apiModels.forEach(model => {
      ...
      const config: ModelCreditsConfig = {
        ...
        created_at: '',
        updated_at: null,
        showDuration: model.parameters?.showDuration !== false,
        showResolution: model.parameters?.showResolution !== false,
        config_id: model.config_id, // #680 添加 config_id 用于获取服务商名称
      };
    ...
    const checkLogin = async () => {
      try {
        ...
        if (typeof window !== 'undefined') {
          ...
          if (userStr) {
            try {
              ...
              // 验证是否是管理员
              if (user.phone === process.env.NEXT_PUBLIC_ADMIN_PHONE) {
                setCurrentUser(user);
                fetchData();
        ...
        if (data.success && data.user) {
          // 验证是否是管理员
          if (data.user.phone !== process.env.NEXT_PUBLIC_ADMIN_PHONE) {
            router.push('/');
            return;
          }
  ...
  function SortableTableRow({ config, modelType }: { config: any; modelType: 'image' | 'video' | 'tool' }) {
    ...
    if (config.resolutions && Array.isArray(config.resolutions) && config.resolutions.length > 0) {
      ...
      const inferredParams = inferParameters(config.credits || 10, modelType === 'video' ? 'video_generation' : 'image_generation', null, config.model_key);
    ...
    const updateCredits = async (label: string, credits: number) => {
      ...
      // 构建更新参数（同时更新 videoPricing 用于视频模型）
      const currentModel = apiModels.find(m => m.id === config.id);
      const currentParams = currentModel?.parameters || {};
      const newParams: any = { ...currentParams, resolutions: newRes };
      
      // 如果是视频模型，同步更新 videoPricing
      if (modelType === 'video' && label) {
        const existingPricing = currentParams.videoPricing || {};
        newParams.videoPricing = {
          ...existingPricing,
          [label]: credits,
        };
      }
      ...
      // 发送到服务器保存 - 同时更新 resolutions, videoPricing 和 credits_base
    ...
    return (
      <TableRow ref={setNodeRef} style={style}>
        ...
        <TableCell>
          {(() => {
            const configName = apiConfigs?.find(c => c.id === config.config_id)?.name || '';
            const provider = extractProviderShortName(configName);
            return provider ? (
        ...
        {/* 根据模型类型显示不同列 */}
        {modelType === 'image' && (() => {
          // 动态渲染分辨率列 - 与表头一致，按数值排序
          const allResLabels = sortResolutionsByValue([...new Set(
            imageModelConfigs.flatMap((c: any) => (c.resolutions || []).map((r: any) => r.label || r.value))
          )].filter(Boolean)) as string[];
          return (
            <>
              {allResLabels.map((label: string) => (
                <TableCell key={label} className="text-center align-middle">{renderCreditsCell(label)}</TableCell>
              ))}
            </>
          );
        })()}
        {modelType === 'video' && (() => {
          // #548 按计费模式区分：固定计费（Veo/Sora）vs 按秒计费（Seedance）
          const isPerSecond = config.showDuration;
          const modelResolutions = config.resolutions || [];
          
          if (isPerSecond) {
            // 按秒计费 - 显示每个分辨率的积分/秒，按数值排序
            const allResLabels = sortResolutionsByValue([...new Set(
              videoModelConfigs.filter((c: any) => c.showDuration).flatMap((c: any) => (c.resolutions || []).map((r: any) => r.label || r.value))
            )].filter(Boolean)) as string[];
            return (
              <>
                {allResLabels.map((label: string) => (
            ...
            // 固定计费 - 显示每个分辨率的积分/次，按数值排序
            const allResLabels = sortResolutionsByValue([...new Set(
              videoModelConfigs.filter((c: any) => !c.showDuration).flatMap((c: any) => (c.resolutions || []).map((r: any) => r.label || r.value))
            )].filter(Boolean)) as string[];
            if (allResLabels.length > 0) {
              return (
                <>
                  {allResLabels.map((label: string) => (
                    <TableCell key={label} className="text-center align-middle">{renderCreditsCell(label)}</TableCell>
            ...
            return (
              <>
                <TableCell className="text-center align-middle">
                  <input
                    type="number"
                    defaultValue={config.credits_base || config.credits || 80}
                    onBlur={async (e) => {
                      const newCredits = Number(e.target.value) || 0;
                      if (newCredits !== (config.credits_base || config.credits)) {
                        await updateModelCredits(config.id, 'credits_base', newCredits);
                      }
                    }}
                    className="w-16 h-7 px-1 text-sm text-center border border-transparent hover:border-blue-400 focus:border-blue-500 rounded outline-none bg-transparent [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
                </TableCell>
              </>
            );
  ...
  // 获取支付维护状态
  const fetchPaymentMaintenance = async () => {
    try {
      const res = await fetch('/api/payment/maintenance');
      const data = await res.json();
      if (data.success) {
        setPaymentMaintenance(data.maintenance);
      }
    } catch (error) {
      console.error('Error fetching payment maintenance:', error);
    }
  };

  // 切换支付维护状态
  const togglePaymentMaintenance = async () => {
    setPaymentMaintenanceLoading(true);
    try {
      const res = await fetch('/api/payment/maintenance', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ maintenance: !paymentMaintenance }),
        credentials: 'include',
      });
      const data = await res.json();
      if (data.success) {
        setPaymentMaintenance(data.maintenance);
        toast.success(data.maintenance ? '支付通道已关闭维护' : '支付通道已恢复');
      } else {
        toast.error(data.error || '操作失败');
      }
    } catch (error) {
      console.error('Error toggling payment maintenance:', error);
      toast.error('操作失败');
    } finally {
      setPaymentMaintenanceLoading(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className={`w-full flex justify-start gap-1 mb-6 h-auto p-1 ${adminDarkMode ? 'bg-gray-800/50' : 'bg-gray-100'} rounded-xl`}>
            ...
            <TabsTrigger value="showcase-review" className="flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <CheckCircle className="h-4 w-4" />
              投稿审核
            </TabsTrigger>
            ...
            <TabsTrigger value="test-center" className="flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
                ...
                <Table>
                  ...
                  <TableBody>
                    {users.map((user, index) => (
                      <TableRow key={user.id || `admin-${index}`} className={user.phone === process.env.NEXT_PUBLIC_ADMIN_PHONE ? (adminDarkMode ? 'bg-purple-900/30' : 'bg-purple-50') : ''}>
          ...
          <TabsContent value="packages">
            <Card className={`border ${adminDarkMode ? 'bg-gray-900/50 border-gray-800' : 'border-gray-200'}`}>
              ...
              <CardContent>
                {/* 支付维护开关 */}
                <div className={`mb-6 p-4 rounded-lg border ${paymentMaintenance ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      {paymentMaintenance ? (
                      ...
                      <div>
                        <p className={`font-medium ${paymentMaintenance ? 'text-red-800 dark:text-red-200' : 'text-green-800 dark:text-green-200'}`}>
                          支付通道状态：{paymentMaintenance ? '维护中' : '正常'}
                        </p>
                        <p className={`text-sm ${paymentMaintenance ? 'text-red-600 dark:text-red-300' : 'text-green-600 dark:text-green-300'}`}>
                          {paymentMaintenance ? '用户将看到"在线支付通道维护"提示，无法在线支付' : '用户可以正常使用在线支付功能'}
                        </p>
                      </div>
                    </div>
                    <Button
                      variant={paymentMaintenance ? 'default' : 'destructive'}
                      size="sm"
                      onClick={togglePaymentMaintenance}
                      disabled={paymentMaintenanceLoading}
                    >
                      {paymentMaintenanceLoading ? (
          ...
          <TabsContent value="model-credits">
            <div className="space-y-6">
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="align-middle">服务商</TableHead>
                            <TableHead className="align-middle">模型标识</TableHead>
                            <TableHead className="align-middle">模型名称</TableHead>
                            {/* 动态分辨率列头 - 从实际模型数据提取，按数值排序 */}
                            {sortResolutionsByValue([...new Set(imageModelConfigs.flatMap((c: any) => (c.resolutions || []).map((r: any) => r.label || r.value)))].filter(Boolean)).map((res: string) => (
                              <TableHead key={res} className="text-center align-middle">{res}</TableHead>
                            ))}
              ...
              <Card className={`border ${adminDarkMode ? 'bg-gray-900/50 border-gray-800' : 'border-gray-200'}`}>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <span className="text-xl">🎬</span> 视频模型（按秒计费）
                    <span className="text-xs text-muted-foreground ml-2">积分 = 分辨率单价(每秒) × 时长(秒)</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <DndContext
                    ...
                    onDragEnd={handleDragEnd}
                  >
                    <SortableContext items={videoModelConfigs.filter(c => c.showDuration).map(c => c.id)} strategy={verticalListSortingStrategy}>
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead className="w-10 align-middle">排序</TableHead>
                            ...
                            <TableHead className="align-middle">模型标识</TableHead>
                            <TableHead className="align-middle">模型名称</TableHead>
                            {/* 动态分辨率列头 - 按秒计费模型，按数值排序 */}
                            {sortResolutionsByValue([...new Set(videoModelConfigs.filter((c: any) => c.showDuration).flatMap((c: any) => (c.resolutions || []).map((r: any) => r.label || r.value)))].filter(Boolean)).map((res: string) => (
                              <TableHead key={res} className="text-center align-middle">{res}/秒</TableHead>
                            ))}
                            <TableHead className="align-middle">描述</TableHead>
                            <TableHead className="text-center align-middle">状态</TableHead>
                            <TableHead className="text-center align-middle">展示</TableHead>
                        ...
                        <TableBody>
                          {videoModelConfigs.filter(c => c.showDuration).map((config) => (
                            <SortableTableRow key={config.id} config={config} modelType="video" />
                          ))}
                          {videoModelConfigs.filter(c => c.showDuration).length === 0 && (
                            <TableRow>
                              <TableCell colSpan={10} className="text-center text-gray-500 py-4">
                                暂无按秒计费视频模型
                              </TableCell>
              ...
              <Card className={`border ${adminDarkMode ? 'bg-gray-900/50 border-gray-800' : 'border-gray-200'}`}>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <span className="text-xl">🎥</span> 视频模型（固定计费）
                    <span className="text-xs text-muted-foreground ml-2">每次生成固定积分，可选分辨率</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <DndContext
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="align-middle">模型标识</TableHead>
                            <TableHead className="align-middle">模型名称</TableHead>
                            {/* 动态分辨率列头 - 固定计费模型，显示每个分辨率的积分/次，按数值排序 */}
                            {sortResolutionsByValue([...new Set(videoModelConfigs.filter((c: any) => !c.showDuration).flatMap((c: any) => (c.resolutions || []).map((r: any) => r.label || r.value)))].filter(Boolean)).map((res: string) => (
                              <TableHead key={res} className="text-center align-middle">{res}/次</TableHead>
                            ))}
                            <TableHead className="align-middle">描述</TableHead>
                            <TableHead className="text-center align-middle">状态</TableHead>
                            <TableHead className="text-center align-middle">展示</TableHead>
          ...
          <TabsContent value="api-keys">
            <Card className={`border ${adminDarkMode ? 'bg-gray-900/50 border-gray-800' : 'border-gray-200'}`}>
                  ...
                  {!selectedConfigId && (
                    <div className="flex items-center gap-2">
                      <Button 
                        variant="outline"
                        size="sm"
                        className="border-amber-300 text-amber-600 hover:bg-amber-50 hover:text-amber-700 dark:border-amber-800 dark:text-amber-400 dark:hover:bg-amber-950 dark:hover:text-amber-300"
                        onClick={async () => {
                          ...
                          try {
                            ...
                            const data = await res.json();
                            if (data.success) {
                              setCircuitBreakerBans(data.activeBans || []);
                            }
                          } catch (err) {
                            // 静默失败
                          }
                      ...
                      >
                        ...
                        {circuitBreakerLoading ? '查询中...' : '查看熔断状态'}
                      </Button>
                      {circuitBreakerBans.length > 0 && (
                        <Button 
                          variant="outline"
                          size="sm"
                          ...
                          onClick={async () => {
                            try {
                              const res = await fetch('/api/system/circuit-breakers', { method: 'POST' });
                              const data = await res.json();
                              if (data.success) {
                                toast.success(`✅ 所有通道熔断状态已重置（清除 ${data.bansCleared} 条熔断 + ${data.failuresCleared} 条失败计数），系统将重新探测上游接口`);
                                setCircuitBreakerBans([]);
                              } else {
              ...
              <CardContent>
                {/* 熔断详情展示 */}
                {circuitBreakerBans.length > 0 && (
                  <div className={`mb-4 rounded-lg border p-4 ${adminDarkMode ? 'bg-red-950/30 border-red-800' : 'bg-red-50 border-red-200'}`}>
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-semibold text-red-600 dark:text-red-400 flex items-center gap-2">
                        <ShieldAlert className="h-4 w-4" />
                        当前熔断中的通道 ({circuitBreakerBans.length})
                      </h4>
                    </div>
                    <div className="space-y-2">
                      {circuitBreakerBans.map((ban, idx) => {
                        const remainingMin = Math.ceil(ban.remainingMs / 60000);
                        const remainingHour = Math.floor(remainingMin / 60);
                        const remainingMinPart = remainingMin % 60;
                        const timeStr = remainingHour > 0 
                          ? `${remainingHour}小时${remainingMinPart}分` 
                          : `${remainingMinPart}分钟`;
                        return (
                          <div key={idx} className={`flex items-center justify-between text-sm rounded px-3 py-2 ${adminDarkMode ? 'bg-gray-900/50' : 'bg-white'}`}>
                            <div className="flex items-center gap-3">
                              <Badge variant="destructive" className="text-xs">{ban.resolution}</Badge>
                              <span className={`font-medium ${adminDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{ban.configName}</span>
                              <span className="text-muted-foreground font-mono text-xs">{ban.keyPrefix}</span>
                              <span className="text-muted-foreground text-xs truncate max-w-[200px]" title={ban.error}>{ban.error}</span>
                            </div>
                            <span className="text-red-500 font-medium whitespace-nowrap">剩余 {timeStr}</span>
                          </div>
                        );
                      })}
                    </div>
                              ...
                              <span className="text-xs text-muted-foreground font-mono">
                                {config.api_key 
                                  ? (() => {
                                      ...
                                      if (activeCount === 0 && inactiveCount === 0) return '(未设置)';
                                      if (activeCount === 1 && inactiveCount === 0) {
                                        // 单密钥且无状态标记，显示密钥预览
                                        const firstKey = lines[0].split('|')[0].trim();
                                        return firstKey.substring(0, 15) + '...';
                                      }
                                      
                                      // 多密钥或带状态标记
                                      const parts: string[] = [];
                                      if (activeCount > 0) parts.push(`${activeCount} 启用`);
                                      if (inactiveCount > 0) parts.push(`${inactiveCount} 停用`);
                                      return parts.join(' / ');
                                    })()
                                  : '(未设置)'}
                              </span>
                          ...
                          .map((model) => {
                            ...
                            return (
                              <TableRow key={model.id}>
                                ...
                                <TableCell>
                                  {(() => {
                                    const configName = apiConfigs?.find(c => c.id === model.config_id)?.name || '';
                                    const provider = extractProviderShortName(configName);
                                    return provider ? (
                                      <Badge variant="outline" className="text-xs">{provider}</Badge>
                                    ) : <span className="text-muted-foreground text-xs">-</span>;
          ...
          {/* 投稿审核 - 用户投稿展示内容的审核管理 */}
          <TabsContent value="showcase-review">
            <ShowcaseReviewInlinePanel adminDarkMode={adminDarkMode} />
          </TabsContent>
          ...
          <TabsContent value="test-center">
            <TestCenter adminDarkMode={adminDarkMode} />
          </TabsContent>
        </Tabs>

        {/* API 配置编辑对话框 */}
        <Dialog open={showConfigDialog} onOpenChange={setShowConfigDialog}>
          <DialogContent className="!max-w-[1000px] w-[85vw] max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <Textarea 
                        ...
                        placeholder={"sk-xxxxxxxx | 1 | 默认分组\nsk-yyyyyyyy | 0 | 备用分组（已停用）"}
                        rows={5}
                        className="font-mono text-sm"
                      />
                      <div className="text-xs text-amber-600 dark:text-amber-500 mt-1 space-y-1">
                        <p>支持多密钥自动轮询。格式：<code className="bg-muted px-1 rounded">API Key | 状态 | 备注</code></p>
                        <p>状态填写 <code className="bg-muted px-1 rounded">1</code> 启用，<code className="bg-muted px-1 rounded">0</code> 停用，留空默认启用</p>
                      </div>
                    </div>
                  </>
                )}
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold flex items-center gap-2">
                      ...
                      {/* #292 负责人只显示负责人积分 */}
                      {selectedUser.isAdmin && admin ? admin.credits : selectedUser.credits}
                    </p>
                  </div>
                  <div>
