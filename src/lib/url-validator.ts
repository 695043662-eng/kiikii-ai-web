 ...
 * URL 安全验证工具
 * 防止 SSRF（服务端请求伪造）攻击
 * #804 双桶分离：同时白名单 1号桶(临时) 和 2号桶(永久)
 */

// #804 从环境变量获取双桶域名
const COS_BUCKET_TEMP = process.env.NEXT_PUBLIC_COS_BUCKET_TEMP || 'kiikii-ai-1412916018';
const COS_BUCKET_PERM = process.env.NEXT_PUBLIC_COS_BUCKET_PERM || COS_BUCKET_TEMP;
const COS_REGION = process.env.NEXT_PUBLIC_COS_REGION || 'ap-hongkong';

// 允许代理的域名白名单
const ALLOWED_PROXY_DOMAINS = [
  // #全局域名大一统 自定义 CDN 域名（CNAME 到 COS Bucket）
  'img.kiikii.me',
  // 腾讯云 COS（公网）
  ...
  '*.myqcloud.com',              // COS 所有区域
  '*.cos.ap-*.myqcloud.com',     // COS 通配
  // #762 腾讯云 COS（内网）- 生产服务器走内网免费流量
  'cos-internal.ap-hongkong.myqcloud.com',
  'cos-internal.ap-guangzhou.myqcloud.com',
  'cos-internal.ap-shanghai.myqcloud.com',
  'cos-internal.ap-beijing.myqcloud.com',
  '*.cos-internal.ap-*.myqcloud.com', // COS 内网通配
  // AI 服务商域名（视频/图片生成回调 URL）
  'api.mmw.ink',
  'grsai.dakka.com.cn',
  'ai.t8star.org',
  'ai.t8star.cn',
  ...
  '*.alicdn.com',                // 阿里 CDN
  // #804 双桶白名单（从环境变量动态生成）
  `${COS_BUCKET_TEMP}.cos.${COS_REGION}.myqcloud.com`,       // 1号桶(临时) 公网
  `${COS_BUCKET_TEMP}.cos-internal.${COS_REGION}.myqcloud.com`,  // 1号桶(临时) 内网
  `${COS_BUCKET_PERM}.cos.${COS_REGION}.myqcloud.com`,       // 2号桶(永久) 公网
  `${COS_BUCKET_PERM}.cos-internal.${COS_REGION}.myqcloud.com`,  // 2号桶(永久) 内网
  // 可根据需要添加更多
];
