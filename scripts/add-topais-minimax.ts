/**
 * 添加 TOPAIS MiniMax-H3 视频模型配置
 * 
 * 功能：
 * 1. 在 api_models 表中添加 topais-minimax-h3 模型记录
 * 2. 关联到现有的 TOPAIS api_configs (config_id=28)
 * 
 * 使用方法：
 * npx tsx scripts/add-topais-minimax.ts
 */
...
async function main() {
  console.log('========================================');
  console.log('添加 TOPAIS MiniMax-H3 视频模型配置');
  console.log('========================================\n');
  ...
  if (existingModel) {
    console.log('❌ topais-minimax-h3 模型已存在：');
  ...
  const { data: model, error: modelError } = await supabase
    ...
    .insert({
      ...
      parameters: {
        ...
        showDuration: true,
        showResolution: true,  // 支持2K/768p可选
