...
const LeftSideBar: React.FC<LeftSideBarProps> = ({
  ...
  return (
    ...
    >
      ...
      <div 
        data-toolbar="true" 
        className="relative transition-all duration-300 ease-out group hover:translate-x-1.5 hover:scale-[1.02]"
      >
          ...
          {tools.map((tool, index) => {
            ...
            return (
              <div key={tool.id} className="relative group/tool">
                ...
                {/* 悬停提示 - 水平右方显示 */}
                <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-2 py-1 bg-gray-800 dark:bg-gray-700 text-white text-xs rounded-lg whitespace-nowrap opacity-0 pointer-events-none group-hover/tool:opacity-100 group-hover/tool:pointer-events-auto transition-[opacity,transform] duration-200 shadow-lg flex items-center gap-2 origin-left scale-95 group-hover/tool:scale-100">
                  <span>{tool.name}</span>
                  {/* 快捷键提示 */}
                  {tool.id === 'select' && (
                    <kbd className="px-1.5 py-0.5 bg-gray-600 rounded text-[10px] font-mono">A</kbd>
                  )}
                  {tool.id === 'hand' && (
                    <span className="flex items-center gap-1 text-gray-300">
                      <kbd className="px-1.5 py-0.5 bg-gray-600 rounded text-[10px] font-mono">S</kbd>
                      <span>/</span>
                      <span>长按</span>
                      <kbd className="px-1.5 py-0.5 bg-gray-600 rounded text-[10px] font-mono">Space</kbd>
                    </span>
                  )}
                  {/* 小三角 */}
                ...
                {tool.id === 'shape' && (
                  <div className="absolute left-full ml-2 top-0 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-600 p-2 opacity-0 pointer-events-none group-hover/tool:opacity-100 group-hover/tool:pointer-events-auto transition-[opacity,transform] duration-200 z-50 min-w-[140px] origin-left scale-95 group-hover/tool:scale-100">
                    {/* 形状组 */}
