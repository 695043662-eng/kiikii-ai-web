...
// #511 已删除 model_credits_config 表
...
// #353 视频收藏表（视频面板 + 视频页面共用）
export const videoFavoritesIdSeq = pgSequence("video_favorites_id_seq", { startWith: "1", increment: "1", minValue: "1", maxValue: "2147483647", cache: "1", cycle: false });
export const videoFavorites = pgTable("video_favorites", {
	...
	index("idx_video_favorites_sort_order").using("btree", table.sortOrder.asc().nullsLast().op("int4_ops")),
	index("idx_video_favorites_user_id").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	pgPolicy("Users can delete own video favorites", { as: "permissive", for: "delete", to: ["public"] }),
	pgPolicy("Users can update own video favorites", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can insert own video favorites", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can view own video favorites", { as: "permissive", for: "select", to: ["public"] }),
]);

// #353 提示词面板收藏表（独立）
export const textPanelFavoritesIdSeq = pgSequence("text_panel_favorites_id_seq", { startWith: "1", increment: "1", minValue: "1", maxValue: "2147483647", cache: "1", cycle: false });
export const textPanelFavorites = pgTable("text_panel_favorites", {
	id: serial().primaryKey().notNull(),
	userId: text("user_id").notNull(),
	content: text().notNull(),
	sortOrder: integer("sort_order").default(0),
	...
	index("idx_text_panel_favorites_sort_order").using("btree", table.sortOrder.asc().nullsLast().op("int4_ops")),
	index("idx_text_panel_favorites_user_id").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	pgPolicy("Users can delete own text panel favorites", { as: "permissive", for: "delete", to: ["public"] }),
	pgPolicy("Users can update own text panel favorites", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can insert own text panel favorites", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can view own text panel favorites", { as: "permissive", for: "select", to: ["public"] }),
]);
...
// #511 已删除 model_credits_config 表（数据已迁移到 api_models）
