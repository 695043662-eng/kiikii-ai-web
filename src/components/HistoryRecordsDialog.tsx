...
export default function HistoryRecordsDialog({ open, onOpenChange, source }: HistoryRecordsDialogProps) {
  ...
  const checkCacheForRecords = async (records: HistoryRecord[]) => {
    try {
      ...
      // 第二层：代理 URL（#524 修复：浏览器直连 COS 超时，改用后端代理）
      if (missedKeys.length > 0) {
        console.log('[HistoryRecords] IndexedDB 未命中，使用代理 URL:', missedKeys.length, '张');
        ...
        for (const key of missedKeys) {
          ...
          if (info) {
            hitCount++;
            // #524 使用代理 URL 替代签名 URL
            const proxyUrl = `/api/canvas/image?key=${encodeURIComponent(key)}`;
            if (!updates.has(info.recordId)) {
              updates.set(info.recordId, []);
            }
            updates.get(info.recordId)!.push({ imageIndex: info.imageIndex, cachedUrl: proxyUrl, type: info.type });
          }
        }
          
          console.log('[HistoryRecords] 代理 URL 已设置:', missedKeys.length, '张');
      }
  ...
  const Img = ({ src, alt, className, onClick, isDisliked }: { 
    ...
    return (
      <div className="relative group">
        <img src={src} alt={alt} className={`${className} ${isDisliked ? 'opacity-40 grayscale' : ''}`} onClick={onClick} loading="lazy" onError={() => handleImageError(src)} referrerPolicy="no-referrer-when-downgrade" />
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="w-[120px]">生成内容</TableHead>
                  ...
                  <TableBody>
                    {paginatedRecords.map((record) => (
                      <TableRow key={record.id}>
                        ...
                        <TableCell className="text-sm font-medium">{record.model_name || record.model || '-'}</TableCell>
                          ...
                          {record.reference_images && record.reference_images.length > 0 ? (
                            <div className="flex gap-1">
                              {record.reference_images.slice(0, 2).map((img, idx) => (
                                <img key={idx} src={img} alt="" className="w-10 h-10 object-cover rounded border cursor-pointer hover:opacity-80" loading="lazy" onClick={() => handlePreviewImage(record.reference_images!, idx)} referrerPolicy="no-referrer-when-downgrade" />
                              ))}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-1 max-w-[200px]">
                              {record.videos.map((videoUrl, idx) => (
                                <div key={idx} className="relative group">
                                  ...
                                  />
                                  <span className="absolute bottom-0 right-0 px-1 py-0.5 bg-black/70 text-white text-[8px] rounded">
                                    视频
                                  </span>
                                </div>
                              ))}
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            {/* #561 视频优先显示播放按钮 */}
                            {record.videos && record.videos.length > 0 && (
                              <Button variant="ghost" size="sm" className="text-xs text-[rgb(139,158,232)]" onClick={() => window.open(record.videos![0], '_blank')} title="播放视频">
                                <Eye className="w-3 h-3" />
                              </Button>
                            )}
                            {record.images?.length > 0 && (
                              <Button variant="ghost" size="sm" className="text-xs text-[rgb(139,158,232)]" onClick={() => handlePreviewImage(record.images, 0)} title="查看图片">
                                <Eye className="w-3 h-3" />
                              </Button>
                            )}
                            <Button
