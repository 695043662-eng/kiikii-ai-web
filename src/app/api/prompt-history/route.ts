...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: records, error } = await client
      ...
      .select('id, prompt, reference_images, reference_image_keys')
      .eq('user_id', userId)
      .not('prompt', 'is', null)
      .neq('prompt', '')
    ...
    // 按提示词去重，保留最新的记录
    const promptMap = new Map<string, { id: number; prompt: string; reference_images: string[]; reference_image_keys: string[] }>();
      ...
      if (record.prompt && record.prompt.trim()) {
        ...
        if (!promptMap.has(key)) {
          promptMap.set(key, {
            ...
            reference_image_keys: (record as any).reference_image_keys || [],
          });
        }
      }
    }
