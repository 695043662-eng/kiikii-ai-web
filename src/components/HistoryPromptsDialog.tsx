...
interface HistoryPromptsDialogProps {
  ...
  onSelectPrompt?: (prompt: string, referenceImages?: string[]) => void;
}
...
// 保存提示词到本地
export function savePromptToLocal(prompt: string, reference_images: string[] = [], id?: number) {
  try {
...
export default function HistoryPromptsDialog({ 
  ...
  const handleOneClickPaste = (item: PromptHistoryItem) => {
    if (onSelectPrompt) {
      onSelectPrompt(item.prompt, item.reference_images);
    }
    onOpenChange(false);
  };
