...
export interface CachedUserInfo {
  ...
  is_active?: boolean;       // #504 禁用状态
  locked_until?: string | null;  // #504 解封时间
  created_at: string;
}
...
export async function fetchUserWithCache(): Promise<CachedUserInfo | null> {
  ...
  try {
    ...
    // 🔥 #886 修复：添加 cache: 'no-store' 防止浏览器缓存 + nocache=1 让服务端跳过缓存
    const response = await fetch('/api/user/info?nocache=1&_t=' + Date.now(), {
      credentials: 'include',
      cache: 'no-store',
    });
