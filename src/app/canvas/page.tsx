    ...
    if (data) {
      try {
        ...
        const addSingleImageToCanvas = async (imgUrl: string, promptText: string) => {
          ...
          const maxGroupRatio = 0.5; // 单图
          
          // 计算图片尺寸（保持宽高比）
          const maxDimension = Math.min(visibleWidth, visibleHeight) * maxGroupRatio;
          ...
          const isOverlapping = (x: number, y: number): boolean => {
            ...
            for (const img of existingImages) {
              const overlaps = !(right <= img.left || x >= img.right || bottom <= img.top || y >= img.bottom);
              if (overlaps) return true;
            }
            return false;
          };
          
          // 🔧 #210 修复：先居中画布，再居中画布容器
          // 1. 居中画布（画布中心）
          const canvasCenterX = CANVAS_WIDTH / 2;
          const canvasCenterY = CANVAS_HEIGHT / 2;
          
          // 2. 居中画布容器（屏幕中心对应的画布坐标）
          const screenCenterX = viewportInfo.containerWidth / 2;
          const screenCenterY = viewportInfo.containerHeight / 2;
          const viewCenterX = (screenCenterX - viewportInfo.panX) / viewportInfo.zoom;
          const viewCenterY = (screenCenterY - viewportInfo.panY) / viewportInfo.zoom;
          
          // 3. 优先使用画布中心，如果与屏幕中心差异过大则使用屏幕中心
          const centerDiffX = Math.abs(canvasCenterX - viewCenterX);
          const centerDiffY = Math.abs(canvasCenterY - viewCenterY);
          const threshold = Math.min(viewportInfo.containerWidth, viewportInfo.containerHeight) / viewportInfo.zoom * 0.3;
          
          let targetCenterX = canvasCenterX;
          let targetCenterY = canvasCenterY;
          
          // 如果用户正在查看画布边缘区域，则使用屏幕中心（避免图片出现在视野外）
          if (centerDiffX > threshold || centerDiffY > threshold) {
            targetCenterX = viewCenterX;
            targetCenterY = viewCenterY;
            console.log('[生图发送] 使用屏幕中心:', { viewCenterX, viewCenterY, threshold });
          } else {
            console.log('[生图发送] 使用画布中心:', { canvasCenterX, canvasCenterY });
          }
          
          let finalLeft = targetCenterX - width / 2;
          let finalTop = targetCenterY - height / 2;
          ...
          if (isOverlapping(finalLeft, finalTop)) {
            ...
            let found = false;
            for (const offset of offsets) {
              const newTop = finalTop - offset;
              if (newTop >= 0 && !isOverlapping(finalLeft, newTop)) {
                finalTop = newTop;
                found = true;
                break;
              }
            ...
            if (!found) {
              for (const offset of offsets) {
                const newTop = finalTop + offset;
                if (newTop + height <= CANVAS_HEIGHT && !isOverlapping(finalLeft, newTop)) {
                  finalTop = newTop;
                  found = true;
                  break;
                }
            ...
            if (!found) {
              for (const offset of offsets) {
                const newLeft = finalLeft - offset;
            ...
            if (!found) {
              for (const offset of offsets) {
                const newLeft = finalLeft + offset;
                if (newLeft + width <= CANVAS_WIDTH && !isOverlapping(newLeft, finalTop)) {
                  finalLeft = newLeft;
                  break;
                }
              }
          ...
          canvas.addElement({
            type: 'image',
            name: promptText ? promptText.substring(0, 15) + '...' : '从生图导入',
            x: finalLeft,
            y: finalTop,
            width,
            height,
          ...
          // 镜头切换到新图片（单图占屏幕 50%）
          const groupCenterX = finalLeft + width / 2;
          const groupCenterY = finalTop + height / 2;
          const fitZoom = Math.min(
            (viewportInfo.containerWidth * maxGroupRatio) / width,
            (viewportInfo.containerHeight * maxGroupRatio) / height,
            1
          );
          const finalPanZoom = Math.max(0.1, Math.min(fitZoom, 1));
          const newPanX = viewportInfo.containerWidth / 2 - groupCenterX * finalPanZoom;
          const newPanY = viewportInfo.containerHeight / 2 - groupCenterY * finalPanZoom;
