 ...
 * 🔥 #849 P0 修复：IP 欺骗防御
 * 
 * 旧漏洞：优先读取 x-forwarded-for 的第一个 IP，该字段可被客户端伪造
 * 攻击者只需在每次请求中设置不同的 X-Forwarded-For 头即可绕过 IP 限流
 * 
 * 修复策略（按可信度从高到低）：
 * 1. cf-connecting-ip — Cloudflare 注入，客户端无法伪造（如果使用 Cloudflare）
 * 2. x-real-ip — Nginx 配置 proxy_set_header X-Real-IP $remote_addr 注入，覆盖客户端值
 * 3. x-forwarded-for 的最后一个 IP — 由最近的受信代理追加，非客户端控制
 *    （XFF 链：client(spoofable), proxy1(trusted), proxy2(trusted) → 取最后一个）
 */
export function extractClientIp(request: Request): string {
  // 1. Cloudflare 连接 IP（最高优先级，Cloudflare 强制覆写，客户端无法伪造）
  const cfConnectingIp = request.headers.get('cf-connecting-ip');
  if (cfConnectingIp && cfConnectingIp.trim()) {
    return cfConnectingIp.trim();
  }

  // 2. Nginx X-Real-IP（Nginx 从 TCP 连接提取，覆盖客户端发送的值）
  const realIp = request.headers.get('x-real-ip');
  if (realIp && realIp.trim()) {
    return realIp.trim();
  }

  // 3. X-Forwarded-For — 取最后一个 IP（由最近的受信代理追加）
  // ⚠️ 绝不取第一个 IP！第一个 IP 是客户端自报的，可被任意伪造
  const forwarded = request.headers.get('x-forwarded-for');
  if (forwarded) {
    const ips = forwarded.split(',').map(ip => ip.trim()).filter(Boolean);
    if (ips.length > 0) {
      // 取最后一个 IP = 最近一跳代理追加的真实 IP
