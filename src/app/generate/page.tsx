...
import { downloadFile, downloadViaProxy } from '@/lib/download';
// 【A 计划】乐观上传 Hook
...
function AspectRatioIcon({ ratio, selected }: { ratio: string; selected?: boolean }) {
  const getDimensions = (ratio: string): { w: number; h: number } => {
    switch (ratio) {
      case '1:1': return { w: 18, h: 18 };
      case '3:4': return { w: 12, h: 16 };
      case '4:3': return { w: 16, h: 12 };
      case '9:16': return { w: 10, h: 18 };
      ...
      case '9:21': return { w: 9, h: 21 };
      case '1:4': return { w: 5, h: 20 };
      case '4:1': return { w: 20, h: 5 };
      case '1:8': return { w: 3, h: 24 };
      case '8:1': return { w: 24, h: 3 };
      case '3:1': return { w: 18, h: 6 };
      case '1:3': return { w: 6, h: 18 };
      case '2:1': return { w: 18, h: 9 };
      case '1:2': return { w: 9, h: 18 };
...
// 基础比例列表（所有图片模型通用，API获取失败时的兜底）#493 移除 auto
...
export default function SingleGeneratePage() {
  ...
  // #509 生图页面违规弹窗（与画布页面共用 Hook）
  const {
    showViolationWarning,
    setShowViolationWarning,
    showBannedDialog: generateBannedDialogVisible,
    setShowBannedDialog: setGenerateBannedDialogVisible,
  ...
  useEffect(() => {
    const fetchModelOptions = async () => {
      try {
        ...
        const data = await fetchConfig('/api/config?service_type=image_generation');
  ...
  // #826 乐观加载：追踪正在压缩中的图片数量（无base64预览时的骨架卡片）
  ...
  const handleImageError = useCallback((imageUrl: string, taskCreatedAt?: Date) => {
    // #525 混合架构：如果当前URL是服务商URL（非代理URL），尝试降级到代理URL
    if (!imageUrl.startsWith('/api/canvas/image')) {
      // 尝试找到对应的 imageKey 来构建代理URL
      setTasks(prev => prev.map(t => {
        const imgIndex = t.images.indexOf(imageUrl);
        if (imgIndex === -1) return t;
        const imageKey = t.imageKeys?.[imgIndex];
        if (!imageKey) return t;
        // 降级到代理URL（#842 移除 _t=${Date.now()}：缓存杀手打穿浏览器缓存）
        const proxyUrl = `/api/canvas/image?key=${encodeURIComponent(imageKey)}`;
        const newImages = [...t.images];
        newImages[imgIndex] = proxyUrl;
        console.log('[生图页面] #525 服务商URL失败，降级到代理URL:', imageKey.substring(0, 30));
      ...
      import('@/lib/presigned-url-cache').then(({ getPresignedUrls }) => {
        const checkCache = async () => {
          ...
          if (missedKeys.length > 0) {
            console.log('[生图页面] IndexedDB 未命中，使用代理 URL:', missedKeys.length, '张');
            
            // #524 修复：使用代理 URL 替代签名 URL（浏览器直连 COS 超时）
            for (const item of missedKeys) {
              const proxyUrl = `/api/canvas/image?key=${encodeURIComponent(item.imageKey)}`;
              setTasks(prev => prev.map(t => {
                if (t.id !== item.taskId) return t;
                const newImages = [...t.images];
                if (item.imageIndex < newImages.length) {
            ...
            console.log('[生图页面] 代理 URL 已设置:', missedKeys.length, '张');
          }
  ...
  useEffect(() => {
    ...
    if (data) {
      try {
        ...
        // #509 修复：支持 imageKey，通过代理 URL 获取图片
        // #524 优化：使用代理 URL 替代签名 URL（浏览器直连 COS 超时）
        const getImageUrlsByKeys = (keys: string[]): string[] => {
          const validKeys = keys.filter(k => k && k.length > 0);
          return validKeys.map(key => `/api/canvas/image?key=${encodeURIComponent(key)}`);
        };
        
        // 支持新格式（多图）和旧格式（单图）
        if (parsed.images && Array.isArray(parsed.images)) {
          // 新格式：多张图片（#509 支持 imageKey）
          const imageKeys = parsed.images.map((img: { imageKey?: string }) => img.imageKey || '').filter((k: string) => k && k.length > 0);
          
          // 优先用 imageKey 生成代理 URL
          if (imageKeys.length > 0) {
            const proxyUrls = getImageUrlsByKeys(imageKeys);
            if (proxyUrls.length > 0) {
              setReferenceImages(proxyUrls);
              setReferenceImageUrls(proxyUrls);
              setReferenceImageKeys(imageKeys);
              setReferenceImageMd5s([]);
              referenceImageUrlsRef.current = proxyUrls;
              referenceImageKeysRef.current = imageKeys;
              referenceImageMd5sRef.current = [];
              console.log('[从画布加载 #509] 通过代理 URL 加载参考图:', proxyUrls.length);
            } else {
                // 代理 URL 为空，降级用 imageUrl
                const imageUrls = parsed.images.map((img: { imageUrl: string; prompt: string }) => img.imageUrl).filter((url: string) => url && !url.startsWith('blob:'));
              ...
              toast.success(`已从画布加载 ${Math.max(proxyUrls.length, imageKeys.length)} 张图片`);
          } else {
            // 没有 imageKey，降级用 imageUrl（过滤 blob URL）
            const imageUrls = parsed.images.map((img: { imageUrl: string; prompt: string }) => {
              if (img.imageUrl?.startsWith('blob:')) blobUrlsToRevoke.push(img.imageUrl);
              return img.imageUrl;
          ...
          console.log('[从画布加载 #509] 处理多图模式');
        } else if (parsed.imageUrl || parsed.imageKey) {
          // 旧格式：单张图片（#509 支持 imageKey）
          const imageKey = parsed.imageKey || '';
          
          if (imageKey) {
            // 有 imageKey，用代理 URL
            const proxyUrls = getImageUrlsByKeys([imageKey]);
            if (proxyUrls.length > 0) {
              setReferenceImages(proxyUrls);
              setReferenceImageUrls(proxyUrls);
              setReferenceImageKeys([imageKey]);
              setReferenceImageMd5s([]);
              referenceImageUrlsRef.current = proxyUrls;
              referenceImageKeysRef.current = [imageKey];
              referenceImageMd5sRef.current = [];
              console.log('[从画布加载 #509] 单图通过代理 URL 加载参考图');
            } else {
              // 代理 URL 为空，降级用 imageUrl
            ...
            // 没有 imageKey，降级用 imageUrl
            if (parsed.imageUrl?.startsWith('blob:')) blobUrlsToRevoke.push(parsed.imageUrl);
            const url = parsed.imageUrl || '';
            if (url && !url.startsWith('blob:')) {
              setReferenceImages([url]);
              setReferenceImageUrls([]);
        ...
        // ⚠️ P0.3 修复：延迟释放 blob URL（等目标页面图片加载完成）
        if (blobUrlsToRevoke.length > 0) {
          setTimeout(() => {
            blobUrlsToRevoke.forEach(url => {
              try { URL.revokeObjectURL(url); } catch {}
            });
          }, 5000); // 5秒后释放，确保图片已加载完成
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (config) {
      ...
      // #493 修复：始终使用新模型的第一个非 auto 比例（auto 被后端映射为 1:1，不符合用户预期）
      const nonAutoRatios = config.aspectRatios.filter((r: string) => r !== 'auto');
      const defaultRatio = nonAutoRatios.length > 0 ? nonAutoRatios[0] : config.aspectRatios[0];
      setAspectRatio(defaultRatio);
    }
  }, [model, setResolution, setAspectRatio]);
  ...
  const handleReferenceImageUpload = async (event: any) => {
    ...
    // #826 乐观加载：立即显示骨架卡片
    setCompressingCount(prev => prev + actualProcessCount);
    
    try {
      // #650 视频模型放宽压缩限制
      const isVideoModelGen = getModelSupportedTypes(model).video;
      const compressionMaxSizeMBGen = isVideoModelGen ? 30 : undefined;
      
      // 调用 Hook 处理文件
      await processUploadFiles(files, {
        existingMd5s: referenceImageMd5s,
        existingUrls: referenceImageUrls,  // #669 虚拟副本：传入已有 URL 用于极速秒传
        existingKeys: referenceImageKeys,  // #669 虚拟副本：传入已有 Key 用于极速秒传
        currentCount: referenceImages.length,
        compressionMaxSizeMB: compressionMaxSizeMBGen,
        ...
        onBackgroundComplete: (result: BackgroundUploadResult) => {
          ...
          if (result.success && result.md5) {
            ...
            // #669 虚拟副本：极速秒传不再返回 success:false，这里只处理真正的上传失败
  ...
  const handleRemoveReferenceImage = (index: number) => {
    // 🔧 #7xx 关键修复：使用函数式更新，避免闭包陷阱
    // 原问题：用户上传图片后快速删除，State 变量可能是旧值，导致删除失败
    // 例如：上传图片2后立即删除，referenceImageMd5s 可能还是旧数组，删除不生效
    
    // 1. 使用函数式更新 State（确保使用最新值）
    setReferenceImages(prev => {
      const newState = prev.filter((_, i) => i !== index);
      // 同步更新 Ref（使用最新的 newState）
      referenceImageUrlsRef.current = referenceImageUrlsRef.current.filter((_, i) => i !== index);
      console.log('[删除参考图 #7xx] State + Ref 同步清理, index:', index, '| 新长度:', newState.length);
      return newState;
    });
    
    setReferenceImageUrls(prev => prev.filter((_, i) => i !== index));
    setReferenceImageMd5s(prev => {
      const newState = prev.filter((_, i) => i !== index);
      // 同步更新 MD5 Ref
      referenceImageMd5sRef.current = referenceImageMd5sRef.current.filter((_, i) => i !== index);
      console.log('[删除参考图 #7xx] MD5 State + Ref 同步清理, index:', index, '| 新长度:', newState.length);
      return newState;
    });
    setReferenceImageKeys(prev => {
      const newState = prev.filter((_, i) => i !== index);
  ...
  const handleStartGeneration = async () => {
    ...
    // #509 验证：账号是否被禁用（违规封禁）
    if (isBanned) {
      console.log('[调试] 账号被禁用，返回');
      if (lockedUntil) {
        const remaining = Math.max(1, Math.ceil((new Date(lockedUntil).getTime() - Date.now()) / 60000));
        toast.error('账号已被禁用', { description: `请 ${remaining} 分钟后再试` });
      } else {
        toast.error('账号已被禁用', { description: '请联系客服' });
      }
      setGenerateBannedDialogVisible(true);
      return;
    }
    ...
    // 保存提示词历史（#453 修复：同时保存 imageKey）
    savePromptToLocal(prompt.trim(), referenceImageUrlsRef.current.length > 0 ? [...referenceImageUrlsRef.current] : [], referenceImageKeysRef.current.length > 0 ? [...referenceImageKeysRef.current] : []);
    ...
    const newTask: GenerationTask = {
      ...
      params: {
        ...
        // #509 修复：reference_images 不再存 base64，存空数组（节省 localStorage 空间）
        // 实际参考图数据通过 reference_image_keys 换取签名 URL
        reference_images: [],
        reference_image_urls: [...referenceImageUrlsRef.current],
        reference_image_md5s: [...referenceImageMd5sRef.current],
        reference_image_keys: [...referenceImageKeysRef.current],
      },
    };
    ...
    const validUrls = referenceImageUrlsRef.current.filter(url => url && url.length > 0);
    ...
    await handleGenerate({
      ...
      onImageReceived: (data) => {
        setTasks(prev => prev.map(t => {
          ...
          const receivedKey = data.imageKey || data.key;
          ...
          if (data.index !== undefined) {
            ...
            // 按 index 插入图片
            newImages[data.index] = displayUrl;
            if (receivedKey) {
              newImageKeys[data.index] = receivedKey;
            }
          } else {
            // 兜底：没有 index 时追加到末尾
            newImages.push(displayUrl);
            if (receivedKey) {
              newImageKeys.push(receivedKey);
            }
          }
          ...
          // #875 修复：保存 providerUrl 用于下载回退（COS 代理失败时使用服务商直链）
          const newProviderUrls = [...(t.providerUrls || [])];
          const receivedProviderUrl = (data as any).providerUrl as string | undefined;
          if (data.index !== undefined) {
            while (newProviderUrls.length <= data.index) {
              newProviderUrls.push('');
            }
            if (receivedProviderUrl) {
              newProviderUrls[data.index] = receivedProviderUrl;
            }
          } else {
            if (receivedProviderUrl) {
              newProviderUrls.push(receivedProviderUrl);
            }
          }
          ...
          return { ...t, images: newImages, imageKeys: newImageKeys, providerUrls: newProviderUrls, itemStatuses: newItemStatuses, itemErrors: newItemErrors };
        }));
        
        // #150 Local-First：后台异步缓存（不阻塞渲染）
        // #492 修复：优先使用 imageKey
        const cacheKey = data.imageKey || data.key;
        if (cacheKey && data.url) {
          import('@/lib/canvas-image-db').then(({ storeImageByKey }) => {
            fetch(data.url)
              .then(res => res.blob())
              .then(blob => {
                if (blob && blob.size > 0) {
                  storeImageByKey(cacheKey, blob, blob.type).then(success => {
                    if (success) {
                      console.log('[生图页面] #150 已缓存到 IndexedDB:', cacheKey);
                    }
                  }).catch(console.error);
                }
              })
              .catch(err => {
                console.error('[生图页面] #150 后台缓存失败:', cacheKey, err);
              });
          }).catch(console.error);
        }
      },
      ...
      // 错误回调
      // #501 修复：保留已有的违规等详细错误信息，不被全局错误覆盖
      onError: (error) => {
        const globalErrorMsg = translateErrorMessage(error.message);
        const isViolation = globalErrorMsg.includes('违规') || error.message.includes('moderation');
        setTasks(prev => prev.map(t => {
          if (t.id !== newTaskId) return t;
          return {
            ...t,
            status: 'failed',
            error: globalErrorMsg,
            images: [],  // #245 失败时清空图片数组，避免显示坏图图标
            itemStatuses: t.itemStatuses.map(() => 'failed'),
            // 保留已有的详细错误（如"内容违规"），仅对未设置错误的项使用全局错误
  ...
  const handleDownload = async (imageUrl: string, index: number) => {
    ...
    try {
      const filename = `KiikiiAI_${Date.now()}_${index + 1}.png`;
      // #876 统一下载代理：后端全能代理（COS双桶+fallbackUrl+Node.js代理），彻底根除CORS和window.open
      const imageKey = selectedTask?.imageKeys?.[index];
      const providerUrl = selectedTask?.providerUrls?.[index];
      
      let success = false;
      if (imageKey) {
        // 有key：走后端/api/download代理（COS双桶+fallbackUrl三层回退）
        success = await downloadViaProxy(imageKey, filename, providerUrl);
      } else {
        // 无key：直接下载URL
        success = await downloadFile(imageUrl, filename);
      }
      
      if (success) {
        toast.success('下载成功', { id: toastId });
      } else {
        toast.error('抱歉，原图片已过期或损坏', { id: toastId });
      }
    } catch (error: any) {
  ...
  const handleRegenerate = async (task: GenerationTask) => {
    ...
    const originalRefImages = task.params.reference_images || [];
    const originalRefKeys = task.params.reference_image_keys || [];
    const originalRefMd5s = task.params.reference_image_md5s || [];
    const originalRefUrls = task.params.reference_image_urls || [];
    
    // #509 修复：绝不发送 base64 给服务商！
    // 优先用 imageKey 换签名 URL，其次用已有的签名 URL，不再降级到 base64
    let refImagesToSend: string[] = [];
    let refIsUrls = false;
    
    if (originalRefKeys.length > 0) {
      // 有 imageKey，换取新的 COS 签名 URL
      try {
        const validKeys = originalRefKeys.filter((k: string) => k && k.length > 0);
        if (validKeys.length > 0) {
          ...
          if (data.success && data.urls) {
            refImagesToSend = validKeys
              .map((key: string) => data.urls[key])
              .filter((url: string) => url && url.length > 0);
            refIsUrls = true;
            console.log('[再次生成] #509 通过 imageKey 换取签名 URL:', refImagesToSend.length);
          }
        }
      } catch (e) {
        console.error('[再次生成] #509 换取签名 URL 失败:', e);
      }
    }
    
    // imageKey 换取失败时，尝试用已有的签名 URL（非 base64）
    if (refImagesToSend.length === 0 && originalRefUrls.length > 0) {
      // 过滤掉 base64 数据（以 data: 开头的都是 base64）
      const validUrls = originalRefUrls.filter((url: string) => url && !url.startsWith('data:'));
      if (validUrls.length > 0) {
        refImagesToSend = validUrls;
        refIsUrls = true;
        console.log('[再次生成] #509 使用已有签名 URL:', validUrls.length);
      }
    }
    
    // 仍然没有有效的 URL？不降级到 base64，发空数组（纯文生图）
    if (refImagesToSend.length === 0) {
      console.warn('[再次生成] #509 无有效参考图 URL，以纯文生图模式发送（不发送 base64）');
    }
    ...
    const newTask: GenerationTask = {
      ...
      params: {
        ...
        // #253 修复：使用下划线命名（与数据库一致）
        // #509 修复：reference_images 不再存 base64，存签名 URL
        reference_images: refImagesToSend,
        reference_image_urls: refImagesToSend,
      ...
      await handleGenerate({
        ...
        onImageReceived: (data) => {
          setTasks(prev => prev.map(t => {
            ...
            const newImageKeys = [...(t.imageKeys || [])];
            // #492 修复：优先读取 imageKey（后端 SSE 发送的字段名），兼容 key
            const receivedKey2 = data.imageKey || data.key;
            // #525 混合架构：优先使用服务商URL（providerUrl）
            const displayUrl2 = (data as any).providerUrl || data.url;
            if (data.index !== undefined) {
              while (newImages.length <= data.index) {
                newImages.push('');
              }
              ...
              newImages[data.index] = displayUrl2;
              if (receivedKey2) {
                newImageKeys[data.index] = receivedKey2;
              }
            } else {
              newImages.push(displayUrl2);
              if (receivedKey2) {
                newImageKeys.push(receivedKey2);
              }
            }
          ...
          setTasks(prev => {
            ...
            if (result.imageUrls && result.imageUrls.length > 0) {
              saveHistoryRecord({
                ...
                referenceImages: refImagesToSend,  // #509 使用签名 URL，不再用 base64
        ...
        onError: (error) => {
          ...
          toast.error('生成失败', { description: globalErrorMsg });
        },
      });
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
            ...
            >
              <div className="flex items-center gap-2">
                <img src={(() => {
                  ...
                  const isSeedanceModel = ['seedance2', 't8seedance'].includes(ModelDetector.getFamily(mid));
                  const isVeoModel = mid.includes('veo');
                  const isSoraModel = mid.includes('sora');
                  const isGptImage2Model = model.includes('gpt-image-2') || mid.includes('gptimage2');
                  const isNanoBananaModel = mid.includes('nana') || mid.includes('banana');
                  if (isGeminiModel) return '/veo-logo.png';
                  if (isGpt5Model) return '/gpt-image-2-logo.png';
                  if (isSeedanceModel) return '/seedance-logo.png';
                  if (isVeoModel) return '/veo-logo.png';
                  if (isSoraModel) return '/gpt-image-2-logo.png';
                  if (isGptImage2Model) return '/gpt-image-2-logo.png';
                  if (isNanoBananaModel) return '/banana-logo.png';
                  return '/model-logo.png';
                })()} alt="" className={`w-5 h-5 rounded ${(() => {
                  const mid = model.toLowerCase();
                  // 深色logo在夜间模式需要白色处理
                  const needWhite = !mid.includes('banana') && !['seedance2', 't8seedance'].includes(ModelDetector.getFamily(mid));
                  return needWhite ? 'dark:brightness-0 dark:invert' : '';
                })()}`} referrerPolicy="no-referrer-when-downgrade" />
          ...
          <div className="mb-4 flex-shrink-0">
            ...
            >
              ...
              {(() => {
                ...
                <div 
                  key={idx} 
                  className="relative aspect-square cursor-pointer group select-none" 
                ...
                >
                  {/* 图片序号角标 */}
                  <span className="absolute top-1 left-1 px-1.5 py-0.5 text-[10px] font-bold rounded bg-blue-500/80 text-white z-10 select-none">图{idx + 1}</span>
                  <img src={src} alt={`参考图${idx + 1}`} className="w-full h-full object-contain rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 pointer-events-none" draggable={false} referrerPolicy="no-referrer-when-downgrade" />
                  {/* #048 新增：上传中显示加载转圈 */}
              ...
              {/* #826 乐观加载：压缩中的骨架卡片 */}
              {compressingCount > 0 && Array.from({ length: compressingCount }).map((_, i) => (
                <div key={`compressing-skeleton-${i}`} className="relative aspect-square rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 animate-pulse flex flex-col items-center justify-center gap-1">
                  <Loader2 className="w-6 h-6 text-gray-400 dark:text-gray-500 animate-spin" />
                  <span className="text-[10px] text-gray-400 dark:text-gray-500">压缩中</span>
                </div>
              ))}
            ...
            <input id="single-ref-upload" type="file" accept="image/*" multiple onChange={handleReferenceImageUpload} className="hidden" disabled={referenceImages.length + compressingCount >= 6} />
          </div>

          {/* 提示词 */}
          <div className="mb-4 flex-shrink-0 mt-8">
            <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  ...
                  onClick={() => {
                    if (qualityButtonRef.current) {
                      ...
                      setQualityButtonLeft(rect.left);
                      setQualityButtonBottom(window.innerHeight - rect.top + 8);
                    }
        ...
        <div className="flex-1 flex flex-col min-w-0 gap-3 h-full relative">
          ...
          <div className="flex-1 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden flex min-h-0">
            ...
            <div 
              ...
              style={{ userSelect: 'none', WebkitUserSelect: 'none' }}
            >
              {/* #245 强化渲染拦截：必须确保有任意一张图片已生成，避免显示坏图图标 */}
              {/* #498 修复：改为检查是否有任意图片已生成，而不是只检查第一张 */}
              {selectedTask && selectedTask.images?.some(img => img && img.length > 0) ? (
                (() => {
                  // #498 改进：找到已生成的第一张图片作为 fallback，而不是只看 images[0]
            ...
            {selectedTask && (
              <div className="w-56 border-l border-gray-200 dark:border-gray-800 p-3 bg-gray-50 dark:bg-gray-800 overflow-y-auto flex-shrink-0">
                ...
                <div className="mb-3">
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-white mt-1">{modelDisplayNames[selectedTask.params.model] || formatModelName(selectedTask.params.model)}</p>
                </div>
                  ...
                  <div className="grid grid-cols-5 gap-1 mt-1">
                    ...
                    {selectedTask.params.reference_images?.slice(0, 10).map((img: string, idx: number) => (
                      <div key={idx} className="aspect-square rounded overflow-hidden bg-gray-100 dark:bg-gray-700 cursor-pointer border border-gray-200 dark:border-gray-600" onClick={() => setPreviewImage(img)}>
                        <img src={img} alt={`参考图${idx + 1}`} className="w-full h-full object-contain" referrerPolicy="no-referrer-when-downgrade" />
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <HistoryPromptsDialog
        ...
        onSelectPrompt={async (selectedPrompt, selectedRefImages, selectedRefImageKeys) => {
          setPrompt(selectedPrompt);
          if (selectedRefImages && selectedRefImages.length > 0) {
            // #453 检查是否有 imageKey，优先使用 key 获取新的签名 URL
            if (selectedRefImageKeys && selectedRefImageKeys.length > 0) {
              console.log('[历史记录] 使用 imageKey 获取新签名 URL:', selectedRefImageKeys);
              try {
                const response = await fetch('/api/canvas/signed-url', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ keys: selectedRefImageKeys }),
                });
                const result = await response.json();
                if (result.success && result.urls) {
                  const newUrls = selectedRefImageKeys.map((key: string) => result.urls[key]).filter(Boolean);
                  console.log('[历史记录] 获取到新签名 URL:', newUrls.length, '张');
                  setReferenceImageUrls(newUrls);
                  setReferenceImageKeys([...selectedRefImageKeys]);
                  referenceImageUrlsRef.current = newUrls;
                  referenceImageKeysRef.current = [...selectedRefImageKeys];
                } else {
                  console.warn('[历史记录] 获取签名 URL 失败，使用原 URL');
                ...
                console.error('[历史记录] 获取签名 URL 异常:', error);
                setReferenceImageUrls(selectedRefImages);
                setReferenceImageKeys([...selectedRefImageKeys]);
                referenceImageUrlsRef.current = [...selectedRefImages];
                referenceImageKeysRef.current = [...selectedRefImageKeys];
              ...
              setReferenceImageMd5s([]); // 历史记录中没有 MD5
              referenceImageMd5sRef.current = [];
            } else {
              // 没有 imageKey，使用原 URL（可能已过期）
              console.log('[历史记录] 无 imageKey，使用原 URL:', selectedRefImages.length, '张');
              setReferenceImageUrls(selectedRefImages);
              setReferenceImages([]); // 清空 base64，使用 URL 模式
              setReferenceImageKeys([]); // 没有 key
              setReferenceImageMd5s([]); // 没有 MD5
              referenceImageUrlsRef.current = [...selectedRefImages];
              referenceImageKeysRef.current = [];
              referenceImageMd5sRef.current = [];
            }
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-[360px] max-h-[60vh] overflow-hidden">
              ...
              <div className="p-2 space-y-1 overflow-y-auto max-h-[50vh]">
                {modelOptions.map((modelId) => {
                  ...
                  const isSeedanceModel = ['seedance2', 't8seedance'].includes(ModelDetector.getFamily(modelId));
                  const isVeoModel = modelId.toLowerCase().includes('veo');
                  const isSoraModel = modelId.toLowerCase().includes('sora');
                  const isGptImage2Model = modelId.includes('gpt-image-2') || modelId.toLowerCase().includes('gptimage2');
                  const isNanoBananaModel = modelId.toLowerCase().includes('nana') || modelId.toLowerCase().includes('banana');
                  const modelLogo = isGeminiModel ? '/veo-logo.png'
                    ...
                    : '/model-logo.png';
                  // 深色logo在夜间模式需要白色处理（浅色logo不需要：banana/seedance是浅色）
                  const needWhiteLogo = !isSeedanceModel && !isNanoBananaModel;
                  ...
                  if (!config) {
                    return (
                      ...
                      >
                        <img src={modelLogo} alt="" className={`w-8 h-8 rounded-lg ${needWhiteLogo ? 'dark:brightness-0 dark:invert' : ''}`} referrerPolicy="no-referrer-when-downgrade" />
                  ...
                  if (!isActive) {
                    return (
                      ...
                      >
                        <img src={modelLogo} alt="" className={`w-8 h-8 rounded-lg grayscale ${needWhiteLogo ? 'dark:brightness-0 dark:invert' : ''}`} referrerPolicy="no-referrer-when-downgrade" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-[280px]">
              ...
              <div className="p-2 space-y-1">
                {resolutionOptions.map((res) => {
                  ...
                  const banExpiry = currentModelBannedResolutions?.[res.size?.toUpperCase()];
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-3 rounded-lg text-left transition-colors flex items-center justify-between ${
                      isBanned ? 'bg-gray-50 dark:bg-gray-900 opacity-50 cursor-not-allowed' :
                      resolution === res.size 
                  ...
                  >
                    <div>
                      <div className="text-sm font-medium">
                        {res.size}
                        {isBanned && <span className="text-xs text-red-500 ml-1">拥挤{banRemaining > 0 ? `(${banRemaining}分)` : ''}</span>}
                      </div>
                    </div>
                    <div className={`text-xs ${isBanned ? 'text-gray-400' : (resolution === res.size ? 'text-gray-300' : 'text-gray-500')}`}>
                      {res.credits} 积分
                    </div>
                  </button>
                  );
      ...
      {showQualityPicker && (model.startsWith('t8star.') || model === 'gpt-image-2-vip' || model === 'gpt-image-2') && (
        <>
          ...
          <div className="fixed z-50" style={{ left: qualityButtonLeft, bottom: qualityButtonBottom }}>
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-[200px]">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
                <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100">选择品质</h3>
                <button 
                  className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" 
      ...
      {/* #509 生图页面禁用弹窗 */}
      <Dialog open={generateBannedDialogVisible} onOpenChange={(open) => {
        if (!open && isBanned && lockedUntil) return;
        setGenerateBannedDialogVisible(open);
      }}>
        <DialogContent className="sm:max-w-md" showCloseButton={false}>
          <DialogHeader>
          ...
          <DialogFooter className="sm:justify-center">
            <Button
              variant="destructive"
              onClick={() => setGenerateBannedDialogVisible(false)}
              className="w-full"
            >
              我知道了
            </Button>
