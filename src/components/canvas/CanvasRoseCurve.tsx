 ...
 * - 容器太小时自动隐藏细节
 */
const CanvasRoseCurve = memo(function CanvasRoseCurve({ color = '#e84393', className = '', showDetail = false, gradientBg = true }: CanvasRoseCurveProps) {
  ...
  const pctFont = Math.max(5, minDim * 0.028);          // 百分比字号 占 2.8%
  ...
  return (
    <div ref={wrapperRef} className={`w-full h-full flex flex-col items-center justify-center relative overflow-hidden ${className}`}>
      {/* 渐变闪烁光效 - 直接放在组件内部 */}
      {gradientBg && (
      <div style={{
        position: 'absolute',
      ...
      {showDetailSection && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap, marginTop: minDim * 0.06, position: 'relative', zIndex: 1, width: '100%', overflow: 'hidden' }}>
          ...
          <div ref={pctRef} style={{ color, fontSize: pctFont, opacity: 0.35, fontVariantNumeric: 'tabular-nums', textAlign: 'center' }}>0%</div>
        </div>
      )}
    </div>
  );
