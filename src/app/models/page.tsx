    ...
    try {
      ...
      if (data.success && data.data?.models) {
        ...
        const convertedModels: ModelDefinition[] = apiModels.map(model => {
          ...
          const isTool = modelId.includes('smart_split') || 
          ...
          if (isVideo && model.parameters?.durations) {
            // 视频模型：从数据库 durations 获取积分
            resolutions = model.parameters.durations.map((d: any) => ({
              size: d.label || d.value || '视频',
              credits: d.credits || model.credits_base,
            }));
          } else if (!isTool) {
                ...
                <div className="relative" style={{ minWidth: '1100px' }}>
                  {imageModels.map((model) => (
                    <div key={model.name} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50" style={{ position: 'relative', height: '48px' }}>
                      <div className="font-mono text-sm text-gray-900 dark:text-white" style={{ position: 'absolute', left: imageColPositions.model, top: '50%', transform: 'translateY(-50%)', width: '480px', paddingLeft: '12px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{model.displayName}</div>
            ...
            <div className="bg-white dark:bg-[#2A2C3F] rounded-xl border border-gray-200 dark:border-transparent w-4/5 mx-auto">
              ...
              <div className="relative overflow-x-auto">
                <div className="flex bg-gray-50 dark:bg-gray-800/30 relative" style={{ minWidth: '1270px', height: '48px' }}>
                  ...
                  <div className="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider text-center cursor-default select-none" style={{ position: 'absolute', left: videoColPositions.dur5s, top: '50%', transform: 'translateY(-50%)', width: '60px' }}>5秒</div>
                  <div className="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider text-center cursor-default select-none" style={{ position: 'absolute', left: videoColPositions.dur10s, top: '50%', transform: 'translateY(-50%)', width: '60px' }}>10秒</div>
                  ...
                  <div className="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider cursor-default select-none" style={{ position: 'absolute', left: videoColPositions.desc, top: '50%', transform: 'translateY(-50%)', width: '400px', paddingLeft: '12px' }}>说明</div>
                ...
                <div className="relative" style={{ minWidth: '1000px' }}>
                  {videoModels.map((model) => (
                    <div key={model.name} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50" style={{ position: 'relative', height: '48px' }}>
                      <div className="font-mono text-sm text-gray-900 dark:text-white" style={{ position: 'absolute', left: videoColPositions.model, top: '50%', transform: 'translateY(-50%)', width: '180px', paddingLeft: '12px' }}>{model.displayName}</div>
                      <div className="flex justify-center items-center text-sm font-medium text-blue-600/95" style={{ position: 'absolute', left: videoColPositions.dur5s, top: '50%', transform: 'translateY(-50%)', width: '60px' }}>{model.resolutions.find(r => r.size === '5秒' || r.size === '5s')?.credits || '-'}</div>
                      <div className="flex justify-center items-center text-sm font-medium text-blue-600/95" style={{ position: 'absolute', left: videoColPositions.dur10s, top: '50%', transform: 'translateY(-50%)', width: '60px' }}>{model.resolutions.find(r => r.size === '10秒' || r.size === '10s')?.credits || '-'}</div>
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-300 truncate" style={{ position: 'absolute', left: videoColPositions.desc, top: '50%', transform: 'translateY(-50%)', width: '400px', paddingLeft: '12px' }}>{model.description}</div>
            ...
            <div className="relative overflow-x-auto">
              ...
              <div className="relative" style={{ minWidth: '1270px' }}>
                {toolModels.map((model) => (
                  <div key={model.name} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50" style={{ position: 'relative', height: '48px' }}>
                    <div className="font-mono text-sm text-gray-900 dark:text-white" style={{ position: 'absolute', left: 80, top: '50%', transform: 'translateY(-50%)', width: '180px', paddingLeft: '12px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{model.displayName}</div>
