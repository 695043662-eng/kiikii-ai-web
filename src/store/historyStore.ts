...
export const useHistoryStore = create<HistoryState>((set, get) => ({
  ...
  fetchRecords: async (limit = 100, offset = 0) => {
    ...
    try {
      const response = await fetch(
        `/api/generation-records?limit=${limit}&offset=${offset}`,
        { credentials: 'include' }
      );
  ...
  clearAllRecords: async () => {
    ...
    try {
      ...
      console.log('[HistoryStore] 已清空所有记录');
    } catch (error) {
...
export const historyStore = {
  ...
  fetchRecords: (limit?: number, offset?: number) => 
    useHistoryStore.getState().fetchRecords(limit, offset),
};
