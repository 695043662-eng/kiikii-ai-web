...
### 症状（军师复盘 #897 发现两大隐患）
1. **主动清空死锁**：用户 Ctrl+A 全选删除（或删掉最后一个元素）→ 前端 #897 空数据保护拦截 → 云端旧元素永远无法清空，刷新后"复活"，陷入数据死锁
2. **手册虚报**：#897 手册声称"服务端空画布覆盖非空云端 → 409 拒绝（纵深防御）"，但 route.ts 中**该代码从未落地**（只有前端一层拦截）——前端拦截一旦被绕过（历史版本页面/其他客户端），云端唯一备份直接被空数据覆盖

### 根因
- #897 的空数据保护无法区分「异常清空」（账号切换/登出链清空）与「用户主动清空」（合法业务动作）
- 写手册时把"计划中的服务端防御"当成了"已完成"，无代码佐证

### 修复（三件套）
| 层 | 文件 | 修复 |
|----|------|------|
| 主动清空打标 | CanvasContext.tsx | 新增 `intentionalClearRef`；`deleteSelected`（选中数 ≥ 全部元素数）与 `deleteElement`（删除最后一个元素）时置 true |
| 放行 + 透传 | useAutoSave.ts | doSave 空数据保护处：标志为 true → 一次性消费（用后即焚）并放行，payload 携带 `is_intentional_clear: true`；否则维持拦截 |
| 服务端真实防线 | autosave/route.ts | UPSERT 前解压入参统计元素数：为空且**无** `is_intentional_clear` 标志 → 查云端元素数（解压失败保守按非空）→ 非空即 **409 EMPTY_CANVAS_OVERWRITE_BLOCKED**；带标志则放行（仍需通过 CAS 校验） |

### 连带修复（顺手挖出的真 Bug）
- **CAS 冲突返回解压格式错误**：route.ts 用 `decompressFromUTF16` 解压云端数据，但前端写入用 `compressToBase64` → 解压必返 null → `JSON.parse('{}')` → 冲突弹窗返回**空数据**，用户点"使用云端数据"即丢画布。已提取 `decompressCanvasData()` 兼容函数（Base64 优先，UTF16 兜底）统一复用

### 军师建议同步落地
| 建议 | 实现 |
|------|------|
| 多 Tab 复核风暴防抖 | onOtherTabLogout 复核结果写入 `kiikii_session_review`（3 秒 TTL），其他 Tab 复核前先读缓存直接采信，N 个 Tab 误报时 `/api/user/info` 从 N 次降为 1 次；复核结果 valid/invalid/uncertain 三态，网络异常不写缓存 |
| 渲染期 ref 反模式 | userIdRef/isLoggedInRef 改用 `useLayoutEffect` 同步（commit 阶段、绘制前，语义等价且符合 React 并发规范），修正 #897 教训第 2 条的表述 |

### 验证记录（test_run + 负向序列互补）
| 用例 | 结果 |
|------|------|
| lint / ts-check / service probe | ✅ 全绿 |
| 非空保存 | 200 ✅ |
| 空画布**无标志**（云端非空） | **409 EMPTY_CANVAS_OVERWRITE_BLOCKED** ✅ 拦截生效 |
| 空画布带 `is_intentional_clear` | 200 ✅ 主动清空放行 |
| 云端数据未被无标志空数据污染 | ✅ |

### 关键教训
1. **防线必须区分"谁在清空"**：异常清空（等待态）拦截、主动清空（业务动作）放行，一刀切拦截会把合法场景锁死成数据死锁
2. **手册记录必须有代码佐证**：写"已落地"前 grep 确认代码存在，否则就是虚报（#897 纵深防御即虚报案例）
3. **压缩格式写入与读取必须同源**：`compressToBase64` 写入配 `decompressFromUTF16` 读取是静默数据丢失，解压必须兼容 + 降级链
4. **test_run 无法表达"期望 4xx"负向用例**：负向拦截验证用 exec_shell 序列互补（写入非空 → 空覆盖 409 → 数据完好 → 带标志放行），正向全绿 + 负向互补 = 完整证据链
