...
CREATE INDEX IF NOT EXISTS idx_video_favorites_sort_order ON video_favorites(sort_order);
...
CREATE INDEX IF NOT EXISTS idx_text_panel_favorites_user_id ON text_panel_favorites(user_id);
CREATE INDEX IF NOT EXISTS idx_text_panel_favorites_sort_order ON text_panel_favorites(sort_order);
...
VALUES (
  'GRS AI 生图',
  'image_generation',
  'https://grsai.dakka.com.cn/v1/api/generate', -- #446 统一使用通用网关接口
  '<见.env.local GRS_API_KEY>',
  'POST',
  '{"Content-Type": "application/json"}',
  '{"model": "${model}", "prompt": "${prompt}", "aspectRatio": "${aspectRatio}", "imageSize": "${imageSize}", "shutProgress": true, "urls": "${urls}"}', -- #447 添加参考图支持
