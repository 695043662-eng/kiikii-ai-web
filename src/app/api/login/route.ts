...
export async function POST(request: NextRequest) {
  try {
    ...
    // 设置登录状态（JWT Token + 兼容旧 Cookie）
    const cookieStore = await cookies();

    // 签发 JWT Token
    const authToken = await signAuthToken(user.id, { phone });
    ...
    return NextResponse.json({ 
      error: '登录失败，请稍后重试'  // 🔒 P0 脱敏：不泄露内部错误信息
    }, { status: 500 });
  }
}
