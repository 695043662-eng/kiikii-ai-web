...
export function calculateImageGroupLayout(input: LayoutInput): LayoutResult {
  ...
  const { GRID_GAP, MIN_ZOOM, MAX_ZOOM, FIXED_MAX_SIZE, getGridCols, getGridRows, getScreenRatio } = CANVAS_IMAGE_RULES;
  ...
  // 2. 计算屏幕占比（用于镜头计算，不再用于图片大小）
  const screenRatio = getScreenRatio(imageCount);
  
  // 3. 计算单元格尺寸：固定最长边为 1500px
  // 如果有图片尺寸，保持宽高比，最长边缩放到 1500px
