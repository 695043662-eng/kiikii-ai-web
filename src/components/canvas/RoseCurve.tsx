...
interface RoseCurveProps {
  ...
  mini?: boolean;
  showDetail?: boolean;
  /** #655 外部进度覆盖（0-100），传入后覆盖内部12秒循环的百分比数字，动画不变 */
  externalProgress?: number;
}

const RoseCurveAnimation = memo(function RoseCurveAnimation({ color = '#e84393', className = '', mini = false, showDetail = false, externalProgress }: RoseCurveProps) {
      ...
      <div className={`flex flex-col items-center justify-center w-full h-full ${className}`}>
        ...
        {showDetail && (
          <div className="flex flex-col items-center gap-0.5 relative z-[1] px-1 pb-1 w-full">
            ...
            <div ref={pctRef} className="text-[12px]" style={{ color: color, opacity: 0.45, fontVariantNumeric: 'tabular-nums' }}>0%</div>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className={`flex flex-col items-center justify-center w-full h-full ${className}`}>
      ...
      <div ref={pctRef} className="text-[15px] mt-1 relative z-[1]" style={{ color: color, opacity: 0.45, fontVariantNumeric: 'tabular-nums' }}>0%</div>
    </div>
  );
});
