...
// 从数据库获取智能分割的积分配置
// 统一数据源：从 api_configs + api_models 表读取
async function getSmartSplitCredits(): Promise<number> {
  try {
    ...
    const { data: config } = await supabase
      .from('api_configs')
      .select('id')
      .eq('service_type', 'smart_split')
      .eq('is_active', true)
      .single();
    
    if (!config) {
      console.log('[Split] 未找到智能分割配置，使用默认值 5');
      return 5;
    }
...
async function getSmartSplitConfig(): Promise<{ apiEndpoint: string; modelName: string; apiKey: string } | null> {
  try {
    ...
    if (configError || !config) {
      console.log('[Split] No smart_split config found in api_configs, error:', configError);
      return null;
    }
    
    console.log('[Split] Found config:', { id: config.id, name: config.name, service_type: config.service_type });
    ...
    if (modelError || !model) {
      console.log('[Split] No model found for smart_split, error:', modelError);
      return null;
    }
    
    console.log('[Split] Found model:', { model_id: model.model_id, model_name: model.model_name });
    
    // 3. 返回配置
    return {
      apiEndpoint: config.api_endpoint || '',
      modelName: model.model_id || '',
      apiKey: getAvailableApiKey(config.api_key)
    };
  } catch (error) {
